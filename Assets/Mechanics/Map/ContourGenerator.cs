...
public class ContourGenerator : ScriptableObject
{
	...
	{
		GetHeights();
		int cascadeCount = Mathf.CeilToInt(terrain.size.y / contourDistance);
		//levels = new ContourLevel[cascadeCount - 1];
		List<ContourLevel> l = new List<ContourLevel>();

		isoLineResolution = terrain.heightmapResolution - 1;
		...
		{
			var le = GenerateIsoLines(i * contourDistance / terrain.size.y);
	...
	ContourLevel GenerateIsoLines(float level)
	{
		...
		float gap = 1f / terrain.heightmapResolution;
		float hgap = gap * 0.5f;
		float topLeft;
		float bottomLeft;
		float topRight;
