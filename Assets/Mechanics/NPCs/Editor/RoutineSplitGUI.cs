...
static class RoutineSplitGUI
{
    ...
    private static readonly Color kSelectedColor = Color.yellow;
    ...
    private static readonly int s_CascadeButtonId = "s_CascadeButtonId".GetHashCode();
    ...
    /**
        ...
        */
    public static void HandleCascadeSliderGUI(ref float[] normalizedCascadePartitions, ref int selectedCascade, NPCTemplate.RoutineStage[] stages)
    {
        ...
        int buttonControlId = GUIUtility.GetControlID(s_CascadeButtonId, FocusType.Passive);
        ...
        for (int i = 0; i < adjustedCascadePartitions.Length; ++i)
        {
            ...
            int adjustedIndex = i == adjustedCascadePartitions.Length - 1 ? 0 : i;

            if (i == adjustedCascadePartitions.Length - 1) colorIndex = 0;//Start and end are the same object
            else colorIndex = (colorIndex + 1) % kCascadeColors.Length;

            GUI.backgroundColor = adjustedIndex == selectedCascade ? kSelectedColor : kCascadeColors[colorIndex];
            ...
            // add regions to slider where the cursor changes to Select
            if (s_DragCache == null)
            {
                Rect clickRect = new Rect(
                    currentX + kPartitionHandleExtraHitAreaWidth, cascadeBoxStartY, boxLength - kPartitionHandleExtraHitAreaWidth * 2, kSliderbarHeight);
                EditorGUIUtility.AddCursorRect(clickRect, MouseCursor.Link, buttonControlId);
                if (clickRect.Contains(currentEvent.mousePosition))
                {
                    if (i == adjustedCascadePartitions.Length - 1) hotButtonIndex = 0;
                    else hotButtonIndex = i;
                }
            }
            ...
            var cascadeText = string.Format("{0}\n{1}", stages[adjustedIndex].activity, stages[adjustedIndex].location);
        ...
        eventType = currentEvent.GetTypeForControl(buttonControlId);
        ...
        switch (eventType)
        {
            case EventType.MouseDown:
                if (hotButtonIndex >= 0)
                {
                    selectedCascade = hotButtonIndex;
                    currentEvent.Use(); // This seems important
                }
                break;
        }
    }
