...
namespace Armere.PlayerController
{
	public class GroundRocksSpell : Spell
	{
		...
		{
			("Raise Rocks", InputReader.GroundActionMapActions.Attack),
		};

		public GroundRocksSpell(Walking caster, GroundRocksSpellAction action) : base(caster)
		{
			a = action;
		...
		public void Cast(InputActionPhase phase)
		{
			...
			for (int i = 0; i < a.createdRocks; i++)
			{
				...
				caster.c.GetComponent<Health>().StartCoroutine(Move(go, particle, delay, a.raiseSpeed, heightAboveGround));
			}
