...
/// </summary>
public sealed class ComponentSettingsListEditor
{
    /// <summary>
    /// A reference to the <see cref="PostProcessProfile"/> being displayed by this editor.
    /// </summary>
    public WorldObjectData asset { get; private set; }
    ...
    List<ComponentSettingsBaseEditor> m_Editors;
    ...
    public ComponentSettingsListEditor(Editor editor)
    {
    ...
    public void Init(WorldObjectData asset, SerializedObject serializedObject)
    {
        ...
        m_SettingsProperty = serializedObject.FindProperty("componentSettings");
        Assert.IsNotNull(m_SettingsProperty);

        m_EditorTypes = new Dictionary<Type, Type>();
        m_Editors = new List<ComponentSettingsBaseEditor>();

        // Gets the list of all available postfx editors
        var editorTypes = RuntimeUtilities.GetAllTypesDerivedFrom<ComponentSettingsBaseEditor>()
                            .Where(
                                t => t.IsDefined(typeof(ComponentSettingsEditorAttribute), false)
                                  && !t.IsAbstract
                            );
        ...
        {
            var attribute = editorType.GetAttribute<ComponentSettingsEditorAttribute>();
            m_EditorTypes.Add(attribute.settingsType, editorType);
        }

        // Create editors for existing settings
        for (int i = 0; i < this.asset.componentSettings.Count; i++)
            CreateEditor(this.asset.componentSettings[i], m_SettingsProperty.GetArrayElementAtIndex(i));
    ...
    void CreateEditor(WorldObjectDataComponentSettings settings, SerializedProperty property, int index = -1)
    {
        var settingsType = settings.GetType();
        Type editorType;

        if (!m_EditorTypes.TryGetValue(settingsType, out editorType))
            editorType = typeof(DefaultComponentSettingsEditor);

        var editor = (ComponentSettingsBaseEditor)Activator.CreateInstance(editorType);
    ...
    void RefreshEditors()
    {
        ...
        for (int i = 0; i < asset.componentSettings.Count; i++)
            CreateEditor(asset.componentSettings[i], m_SettingsProperty.GetArrayElementAtIndex(i));
    }
        ...
        using (new EditorGUI.DisabledScope(!isEditable))
        {
            ...
            for (int i = 0; i < m_Editors.Count; i++)
            {
                ...
                bool displayContent = EditorUtilities.DrawHeader(
                    ...
                    editor.baseProperty,
                    editor.target,
                    () => ResetComponentSettings(editor.target.GetType(), id),
                    () => RemoveComponentSettings(id)
                    );
            ...
            if (GUILayout.Button("Add effect...", EditorStyles.miniButton))
            {
                ...
                var typeMap = WorldObjectDataManager.instance.settingsTypes;
                foreach (var kvp in typeMap)
                {
                    ...
                    if (!exists)
                        menu.AddItem(title, false, () => AddComponentSettings(type));
                    else
    ...
    void AddComponentSettings(Type type)
    {
        m_SerializedObject.Update();

        var effect = CreateNewComponentSettings(type);
        Undo.RegisterCreatedObjectUndo(effect, "Add Component Settings");
    ...
    void RemoveComponentSettings(int id)
    {
    ...
    void ResetComponentSettings(Type type, int id)
    {
        // Remove from the cached editors list
        m_Editors[id].OnDisable();
        m_Editors[id] = null;
        ...
        // Create a new object
        var newEffect = CreateNewComponentSettings(type);
        Undo.RegisterCreatedObjectUndo(newEffect, "Reset Component Settings");
    ...
    WorldObjectDataComponentSettings CreateNewComponentSettings(Type type)
    {
        var effect = (WorldObjectDataComponentSettings)ScriptableObject.CreateInstance(type);
